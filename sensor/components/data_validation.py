...
class DataValidation:
    ...
    def detect_datadrift(self, base_df, current_df, thereshold = 0.5)->bool:
        try:
            status = True
            report = {}
            for column in base_df.columns:
                ...
                if is_same_dist.pvalue > thereshold:
                    is_found = False
    ...
    def initiate_data_validation(self) ->DataIngestionArtifact:
        try:
            ...
            # check DataDrift - We are checking if two datasets have been derived from the same distribution
            # If there is no drift , both belong to same distribution
