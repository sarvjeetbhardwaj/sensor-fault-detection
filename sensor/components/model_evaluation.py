...
os.makedirs(training_pipeline.SAVED_MODEL_DIR, exist_ok=True)
...
class ModelEvaluation:
    ...
    def initiate_model_evaluation(self)->ModelEvaluationArtifact:
        try:
            ...
            latest_mode = load_object(file_path=latest_model_path)
            train_model = load_object(file_path=train_model_file_path)

            y_train_pred = train_model.predict(df)
            y_latest_pred = latest_mode.predict(df)
            ...
            # save the report
            write_yaml_file(file_path=self.model_eval_config.report_file_path,
